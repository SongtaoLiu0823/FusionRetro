...
def get_prediction(p):
    ...
    try:
        ...
        for beam_idx, node in enumerate(top_k_nodes):
            ...
            try:
                ...
                if num_valid_reactant != len(pred_set):
                    continue
                if len(sms):
                    solution_list.append(sorted(list(sms)))
                    if len(solution_list) == args.beam_size:
        ...
        return solution_list[:args.beam_size]
...
for epoch in trange(args.batch*500, min(args.batch*500 + 500, len(tasks))):
    rank = get_prediction_result(tasks[epoch])
    overall_result[:, 1] += 1
    if rank is not None:
